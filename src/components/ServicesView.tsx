...
export default function ServicesView({ setActiveView }: ServicesViewProps) {
  ...
  return (
    <div className="bg-transparent text-slate-900 dark:text-white min-h-screen py-16 px-4 sm:px-6 lg:px-8 selection:bg-cyan-500/20 relative transition-colors duration-300" style={accentVars("ai")}>
      ...
      <div className="mx-auto max-w-7xl space-y-16">
        ...
        <ScrollReveal>
          <div className="text-center space-y-6 max-w-3xl mx-auto">
            <span className="inline-flex items-center gap-2 rounded-full border border-cyan-500/25 bg-cyan-500/10 px-4 py-2 text-xs font-bold uppercase tracking-widest text-cyan-600 dark:text-cyan-400">
            ...
            <h1 className="display-heading font-sans text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white sm:text-5xl lg:text-6xl">
        ...
        <ScrollReveal delay={0.1}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6" id="services-selector-tabs">
            {AGENCY_SERVICES.map((serv, index) => {
              ...
              const iconColors = {
                "ai-powered-solutions": "text-cyan-600 dark:text-cyan-400",
                "software-development": "text-blue-600 dark:text-blue-400",
                "digital-transformation": "text-emerald-600 dark:text-emerald-400",
                "startup-support": "text-violet-600 dark:text-violet-400"
              };
              ...
              return (
                  ...
                  className={`group relative flex flex-col items-center justify-center p-8 rounded-3xl border text-center transition-all duration-500 select-none cursor-pointer overflow-hidden focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:ring-offset-2 focus:ring-offset-slate-900
                    ${
                      isSelected
                        ...
                        : "bg-white/90 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800/60 hover:border-slate-300 dark:hover:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-900/60 hover:scale-[1.01] shadow-md dark:shadow-none"
                    }
                  `}
                ...
                >
                  ...
                  <div className={`relative p-4 rounded-2xl mb-5 transition-all duration-500 backdrop-blur-sm
                    ${isSelected 
                      ? `bg-slate-100 dark:bg-slate-950/50 ${iconColors[serv.id as keyof typeof iconColors]} scale-110 shadow-lg` 
                      : "bg-slate-100 dark:bg-slate-950/50 text-slate-500 dark:text-slate-400 group-hover:scale-105 group-hover:bg-slate-200 dark:group-hover:bg-slate-950/70"}
                  `}
                  >
                  ...
                  {serv.badge && (
                    <span className={`relative mt-3 px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all duration-300 border
                      ${isSelected 
                        ? `bg-slate-100 dark:bg-slate-950/80 ${iconColors[serv.id as keyof typeof iconColors]} ${borderColors[serv.id as keyof typeof borderColors]}` 
                        : "bg-slate-100 dark:bg-slate-950/50 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-500 group-hover:border-slate-300 dark:group-hover:border-slate-700"}
                    `}>
                      {serv.badge}
                    </span>
                  )}
        ...
        <ScrollReveal delay={0.2}>
          <div
            className="grid grid-cols-1 lg:grid-cols-12 gap-8 p-8 md:p-10 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900 dark:bg-gradient-to-br dark:from-slate-900/60 dark:to-slate-950/60 backdrop-blur-md transition-all duration-500 shadow-xl dark:shadow-none"
          ...
          >
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="space-y-4">
                <span className="font-mono text-[11px] text-cyan-600 dark:text-cyan-400 uppercase tracking-widest font-semibold">
                  Service Overview
                </span>
                <h2 className="font-sans text-3xl font-extrabold text-slate-900 dark:text-slate-100 md:text-4xl leading-tight">
                  {activeService.title}
                </h2>
              </div>
              ...
              <div className="p-5 rounded-2xl bg-slate-100 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/50 space-y-4">
                <span className="font-mono text-[10px] text-slate-500 uppercase tracking-wider block">
                  Why This Service
                </span>

                <div className="space-y-3">
                  <div className="flex items-center gap-3 text-sm text-slate-700 dark:text-slate-300 group">
            ...
            {/* Right */}
            <div className="lg:col-span-7 space-y-6 lg:border-l lg:border-slate-200 dark:lg:border-slate-800/60 lg:pl-8">
                ...
                {activeService.details.map((detail, index) => (
                  <div key={index}>
                    <ScrollReveal delay={index * 0.05}>
                      <div className="flex items-start gap-4 p-4 rounded-2xl bg-slate-100 dark:bg-slate-950/30 hover:bg-slate-200 dark:hover:bg-slate-950/50 border border-slate-200 dark:border-slate-900/60 transition-all hover:border-slate-300 dark:hover:border-slate-700 hover:translate-x-1 cursor-default">
                        ...
                        <span className="text-sm text-slate-700 dark:text-slate-300 font-sans leading-relaxed">
                          {detail}
                        </span>
                      </div>
                    </ScrollReveal>
        ...
        <ScrollReveal delay={0.3}>
          <div className="border-t border-slate-200 dark:border-slate-800/40 py-20">
            <div className="text-center mb-12">
              <span className="eyebrow accent-text">
                Our Process
              </span>
              <h2 className="font-sans text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white sm:text-3xl mt-4">
                How We Deliver Excellence
              </h2>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="group p-6 rounded-3xl border border-slate-200 dark:border-slate-800/60 bg-white/90 dark:bg-slate-900/30 hover:border-cyan-500/30 hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-all shadow-md dark:shadow-none">
                ...
                <h3 className="font-sans font-bold text-lg text-slate-900 dark:text-slate-100 mt-2">
              ...
              <div className="group p-6 rounded-3xl border border-slate-200 dark:border-slate-800/60 bg-white/90 dark:bg-slate-900/30 hover:border-emerald-500/30 hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-all shadow-md dark:shadow-none">
              ...
              <div className="group p-6 rounded-3xl border border-slate-200 dark:border-slate-800/60 bg-white/90 dark:bg-slate-900/30 hover:border-violet-500/30 hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-all shadow-md dark:shadow-none">
                ...
                <span className="text-xs font-bold font-mono text-violet-600 dark:text-violet-400 uppercase tracking-wider">
                  03 / QUALITY
                </span>
                <h3 className="font-sans font-bold text-lg text-slate-900 dark:text-slate-100 mt-2">
                  Continuous Handoff
